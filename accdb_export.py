...
[v2 修正]
    - --out と --commit の整合: git add がハードコードの "src" だったため
      --out 変更時に空振りしていた。出力先に自動追従させた
      （.gitattributes の中身のパスも同様に追従）
    - 抽出に失敗したオブジェクトがある場合は自動コミットを中止。
      src/ は毎回作り直すため、失敗分は git 上「削除」に見え、
      そのままコミットするとミラーが壊れた状態で記録されてしまう
    - .laccdb 検出時に警告（Accessで開いたまま＝保存前の編集は抽出に反映されない）
    - 終了コード: 全件成功 0 / 失敗あり 1（バッチ組み込み用）
    - 出力を UTF-8 に統一（リダイレクト時の ⚠ 等での UnicodeEncodeError を防止）
...
def _utf8_console():
    """リダイレクト時(cp932)に ⚠ 等で落ちないよう stdout/stderr を UTF-8 化。"""
...
def ensure_gitattributes(repo_dir, out_rel):
    """UTF-8/LF を Git に明示。既存があれば触らない。パスは --out に追従。"""
    ...
    content = (
        ...
        f"{out_rel}/**/*.txt        text eol=lf\n"
        f"{out_rel}/_manifest.tsv   text eol=lf\n"
        "*.accdb   binary\n"
        "*.laccdb  binary\n"
    )
    with open(path, "wb") as f:
...
def git_commit(repo_dir, out_rel, msg):
    subprocess.run(["git", "-C", repo_dir, "add", out_rel, ".gitattributes"], check=False)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Access .accdb をGit最適化テキストに抽出する（終了コード 0=成功/1=失敗あり）")
    ...
    p.add_argument("--commit", metavar="MSG",
                   help="抽出後に git add <out> && git commit -m MSG（push はしない。失敗ありなら中止）")
    return p.parse_args()
...
def main():
    ...
    # Access で開いたままだと保存前の編集は抽出に反映されない（読取自体は可能なので警告のみ）
    lock = os.path.splitext(accdb)[0] + ".laccdb"
    if os.path.exists(lock):
        print(f"⚠ .laccdb を検出: {lock}")
        print("  Access がこのDBを開いている可能性があります。保存前の編集は抽出に反映されません。\n")

    out_dir = os.path.abspath(args.out)
    repo_dir = os.path.dirname(out_dir)               # 前提: --out はリポジトリ直下のフォルダ
    out_rel = os.path.relpath(out_dir, repo_dir).replace(os.sep, "/")
    ...
    print("\n=== Git設定 ===")
    ensure_gitattributes(repo_dir, out_rel)
    report(counts, n_tbl, failures, out_dir)

    if args.commit:
        print("\n=== コミット ===")
        if failures:
            print("  ⚠ 抽出失敗があるため自動コミットを中止しました。")
            print(f"    {out_rel}/ は毎回作り直すため、失敗したオブジェクトは git 上『削除』に見えます。")
            print("    git status で差分を確認し、意図どおりの場合のみ手動でコミットしてください。")
        else:
            git_commit(repo_dir, out_rel, args.commit)

    print("\n次の手順:")
    print("  git status        # 差分を確認")
    print(f"  git add {out_rel} .gitattributes")
